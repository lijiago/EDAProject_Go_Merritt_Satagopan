...
|  Variable   |   Units   |Data Range or Description                     | Data Type       |
|-------------|-----------|----------------------------------------------|-----------------|
...
FireSize_plot <- ggplot(States_Wildfire_data, aes(x = STATE, y = ACRES)) +
  ...
  stat_summary(geom = "text", fun = max, vjust = -1, size = 3.5,
               label = c("b", "ab", "a")) +
  labs(x = "State", y = "Acres", title = "Pairwise Relationships between Mean Fire Sizes in California, Idaho, and Nevada") +
  ylim(0, 20000) 
print(FireSize_plot)
...
Based on the p-value that we obtained from the result of the ANOVA test, we concluded that there is a statistically significant difference in large wildland fire sizes for the states with the most large wildland fire occurrences, but not a very strong difference. In the Tukey test, our results show that the fire sizes for the three states were statistically the same. Although we did not get strong results, these test show us that there is room for more questions to try to find trends. For the slight difference we did find, it would be interesting to see why the states did have different fire sizes and if it could be due to different land connectivity, land types, or management practices. 

Q3: 

The Mann-Kendall test and time series was performed on the three categories of fires in California from 1984-2007. The Mann-Kendall test produces a p-value of approximately 0.0025 (<0.05) shows that it is statistically significant and there is a monotonic trend in the occurrences/area of wildfire burns over the years in California. The obtained positive tau value implies that the trend is increasing.The time series analysis also indicated that there is an increasing trend in the wildfires over the years, shown in Fig. 9. 

However, due to inconsistency and large gaps in the datasets of the prescribed and wildland use fires, time series analysis could not be performed on them. To further double-check this, the Mann-Kendall test was performed again and the p-values of prescribed and wildland use fires were 0.93 and 0.68 respectively. Hence, we accept the null hypothesis and can see again that these fires are not stastically significant.

This paves way to further studies that can be done or gathered to understand this increase in wildfire severity and occurrences, which could be due to effects of climate change. 
